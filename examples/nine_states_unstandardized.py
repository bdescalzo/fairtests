...
def _prepare_all_states_dataset(data_source, test_size=0.2):
    ...
    print("[Example] Pass 1/2: counting streamed train/test split...", flush=True)
    ...
    print("[Example] Pass 2/2: building unscaled train/test arrays...", flush=True)
    ...
    for X_state, y_state, g_state in _iter_state_arrays(data_source, states):
        ...
        if n_train > 0:
            X_train_chunk = X_state[train_mask].astype(np.float32, copy=False)
        ...
        if n_test > 0:
            X_test_chunk = X_state[test_mask].astype(np.float32, copy=False)
